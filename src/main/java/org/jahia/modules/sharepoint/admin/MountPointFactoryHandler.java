...
/**
 * MountPointFactoryHandler to setup Sharepoint properties using spring webflow
 **/
public class MountPointFactoryHandler extends AbstractMountPointFactoryHandler<MountPointFactory> {
    ...
    private static final String BUNDLE = "resources.sharepoint-external-data-provider";
    ...
    public void init(RequestContext requestContext) {
        ...
        requestContext.getFlowScope().put("sharepointFactory", mountPointFactory);
    }
    ...
    public Boolean save(MessageContext messageContext) {
        ...
        try {
            ...
            if (available) {
                ...
                MessageBuilder messageBuilder = new MessageBuilder().warning().defaultText(Messages.get(BUNDLE, "serverSettings.sharepoint.mountPointFactory.save.unavailable", locale));
                messageContext.addMessage(messageBuilder.build());
            }
        } catch (RepositoryException e) {
            logger.error("Error saving mount point " + mountPointFactory.getName(), e);
            MessageBuilder messageBuilder = new MessageBuilder().error().defaultText(Messages.get(BUNDLE, "serverSettings.sharepoint.mountPointFactory.save.error", locale));
            messageContext.addMessage(messageBuilder.build());
        }
        return false;
    }
